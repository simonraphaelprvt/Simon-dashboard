...
- **Business-Cockpit** (Bento-Grid `.cockpit`): Akquise (Lead-Pipeline-Gauges aus Notion) · Revenue-Chart · Finanzen (Zoho-Gauges) · Zeit (Wochenlast + Kalender-Embed + nächster Dreh) · Pipeline-Radar · Annunciator-Lampen.
...
- **Akquise-Gauges = Lead Radar Pipeline** (Notion-DB `8960e2760b474af1b1df5585cd439894`, muss mit Integration „Simons Dashboard" geteilt sein). `loadLeadMetrics()` zieht via `/api/notion?endpoint=databases/{id}/query` (alte API 2022-06-28 funktioniert), zählt Status (Neu/Angeschrieben/Termin/Kunde/Abgelehnt) → 4 Tachos `ld-neu`/`ld-ang`/`ld-term`/`ld-kunde` (Offen-Neu/Angeschrieben/Termine/Kunden + Conversion), alle 5 Min. Gmail-Login/G-Knopf + `loadGmailMetrics` (Lampen) bleiben, rendern aber nicht mehr in die Gauges (IDs umbenannt). Hinweis: Daten nutzen Bewertung COMMON/INTERESTED/WARM/RAUS (kein „HOT"); Skala = Anteil an Gesamt-Leads.
